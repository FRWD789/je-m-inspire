...
export const editEventSchema = z.object({
  ...
  start_date: z
    .string()
    .refine(val => !isNaN(Date.parse(val)), { message: 'La date de début est invalide' }),

  end_date: z
    .string()
    .refine(val => !isNaN(Date.parse(val)), { message: 'La date de fin est invalide' }),
  ...
  localisation_id: z
    ...
    .refine(val => !isNaN(Number(val)), {
      message: 'L’identifiant de localisation est invalide',
    }),

  categorie_event_id: z
    .union([z.string(), z.number()])
    .refine(val => !isNaN(Number(val)), {
      message: 'L’identifiant de catégorie est invalide',
    }),

  // ✅ Optional file inputs for edit mode
 thumbnail: z
    .any()
    .optional()
    .refine(
